...
from app.services.voxcpm_vc_service import (
    EMOTION_MAP,
    SUPPORTED_EMOTIONS,
    VoxCPMVCUnavailable,
    voxcpm_vc_service,
...
app = FastAPI(
    title="Thai TTS Studio — VoxCPM2 + SeedVC",
    description=(
        "Emotion is cloned from a donor recording by VoxCPM2 (continuation mode), then "
        "SeedVC swaps the timbre onto the target voice. Annotation, segmentation and "
        "assembly are shared with the :8011 studio; generation still runs on the shared "
        "GPU service via the :8020 queue."
    ),
    version="2.0.0",
    lifespan=lifespan,
)
...
def health_check():
    return {
        "status": "ok",
        "service": "thai-tts-voxcpm-seedvc",
        "version": "2.0.0",
        ...
        "pipeline": voxcpm_vc_service.status,
    }
...
async def synthesize_endpoint(req: SynthesizeRequest):
    """Synthesize with the donor -> VoxCPM2 -> SeedVC pipeline.

    ``auto_annotate`` still labels each segment with an emotion; the difference from
    the :8011 studio is what the label does. There it becomes a style parenthetical
    the model reads. Here it selects a donor recording whose delivery VoxCPM2 clones,
    and the parenthetical is stripped before generation.

    Returns 44.1 kHz WAV (SeedVC's rate) with a timestamped filename.
    """
    text = req.text.strip()
    if not text:
        raise HTTPException(status_code=400, detail="Text cannot be empty")
    ...
    try:
        ...
        return Response(
            ...
            headers={
                ...
                # The take is always pinned: SeedVC converts to a named target, so
                # the speaker cannot wander between emotion segments the way it can
                # on the :8011 pipeline.
                "X-Voice-Anchor": "speaker" if req.speaker_id else "reference",
                "X-Donor-Set": (debug[0]["donor_set"] if debug else ""),
            },
        )
    except ValueError as e:
        # An unsupported emotion, or no target voice — both are the caller's to fix.
...
async def synthesize_with_upload_endpoint(
    ...
    """Synthesize onto a one-off uploaded reference clip.

    The upload is SeedVC's *target*: the voice the finished take is in. The emotion
    still comes from the donor set, so this clip needs no particular delivery — a
    calm few seconds of the speaker is ideal.

    Returns 44.1 kHz WAV with a timestamped filename.
    """
...
# ---------------------------------------------------------------------------
# Emotion donors — the recordings VoxCPM2 clones its delivery from
# ---------------------------------------------------------------------------

@app.get("/api/donors")
def list_donors_endpoint():
    """Every donor set on disk, with the emotions it can actually supply.

    A set is one actor across all five emotions. Picking one set for a whole take is
    what keeps the emotion the only thing that changes between chunks — SeedVC
    converts timbre, not accent or speaking style, so mixing actors leaks through.
    """
    return {
        "emotions": list(SUPPORTED_EMOTIONS),
        # What each annotator tone becomes. Ten tones, five recordings — the UI
        # should show this so a substituted emotion is never a surprise.
        "tone_map": EMOTION_MAP,
        "default": settings.default_donor_set or None,
        "sets": voxcpm_vc_service.list_donor_sets(),
    }


@app.get("/api/donors/{donor_set}/{emotion}/audio")
def get_donor_audio_endpoint(donor_set: str, emotion: str):
    """Stream one donor clip, so the emotion can be auditioned before rendering."""
    try:
        wav, _transcript = voxcpm_vc_service.donor_clip(donor_set, emotion)
