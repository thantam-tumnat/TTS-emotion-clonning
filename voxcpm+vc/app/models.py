...
class SynthesizeRequest(BaseModel):
    ...
    # Which actor's recordings supply the emotion. One set is one actor across all
    # five emotions; leaving it unset picks a complete set matching ``gender``.
    donor_set: Optional[str] = Field(
        default=None, description="Emotion donor set id, e.g. 'female_004'"
    )
    gender: Optional[Literal["female", "male"]] = Field(
        default=None, description="Preferred donor gender when donor_set is unset"
    )
