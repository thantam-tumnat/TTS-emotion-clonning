// Pipeline Explorer — donor -> VoxCPM2 (continuation) -> SeedVC, with per-stage playback.
'use strict';
...
// Thai label + one-line hint for each VoxCPM2 knob, in display order.
const TUNE_META = {
  cfg_value:           { label: 'ความเข้มอารมณ์ (cfg)', hint: 'ยิ่งสูงยิ่งยึด delivery ของ donor แรง' },
  inference_timesteps: { label: 'คุณภาพ/สเตป (timesteps)', hint: 'มากขึ้น = คมขึ้น แต่ช้าลง' },
};
...
function renderStages(data) {
  ...
  if (data.tuning) {
    ...
    line.textContent = `ค่า VoxCPM2 ที่ใช้: cfg ${t.cfg_value} · timesteps ${t.inference_timesteps}`;
...
async function run() {
  ...
  setStatus('กำลังรัน VoxCPM2 → SeedVC (อาจใช้เวลา ~20–60 วิ)...', false, true);
  try {
    const tuning = collectTuning();
    const res = await fetch('/api/pipeline/trace', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ donor_set, emotion, speaker_id, text: text || null, tuning }),
    });
