/**
 * Emotion TTS Benchmark & Pipeline Comparison Suite (VoxCPM2 + SeedVC)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function checkHealth() {
    try {
      ...
      if (data.status === 'ok') {
        ...
        const engine = synth.engine || 'VoxCPM2 + SeedVC';
  ...
  async function initPresets() {
    try {
      ...
      // 3. Populate Emotion Checkboxes, then gate them to the dataset's emotions.
  ...
  if (donorSetSelect) donorSetSelect.addEventListener('change', () => {
    renderDonorClipButtons();
    applyDatasetAvailability();
  });
  ...
  // Which emotions the chosen donor actually has clips for. A tone the dataset does
  // not carry can still be generated (it maps to a donor), but it has no clip to
  // audition and is not a first-class dataset emotion, so the UI disables its
  // checkbox and hides its yellow donor-play button.
  function currentDatasetEmotionIds() {
    const setId = getDonorSet();
    if (setId) {
      const s = donorSets.find(x => x.id === setId);
      return new Set((s ? s.emotions : []).map(e => e.id));
    }
    // "Auto by gender": the union of emotions across that gender's sets.
    const g = getGender();
    const ids = new Set();
    donorSets
      .filter(s => !s.gender || s.gender === g)
      .forEach(s => (s.emotions || []).forEach(e => ids.add(e.id)));
    return ids;
  }

  // Enable only the emotions present in the dataset; hide the donor-play button and
  // uncheck+disable the rest. Safe to call before the sets load (empty set = leave
  // everything enabled rather than disabling all).
  function applyDatasetAvailability() {
    const available = currentDatasetEmotionIds();
    document.querySelectorAll('#emotion-checkbox-grid .emotion-pill-chk').forEach(label => {
      const emoId = label.dataset.emotion;
      const input = label.querySelector('input[type="checkbox"]');
      const playBtn = label.querySelector('.emo-donor-play');
      const has = available.size === 0 ? true : available.has(emoId);
      if (playBtn) playBtn.style.display = has ? '' : 'none';
      label.classList.toggle('emotion-unavailable', !has);
      if (input) {
        input.disabled = !has;
  ...
  // NFE Steps Buttons — a picker (not a dropdown) so the step count is one tap,
  // mirroring the repeats picker. The value lives in the hidden #param-steps input
  // everything else already reads, and a dispatched 'change' keeps the echo in sync.
  const stepsPicker = document.getElementById('steps-picker');
  if (stepsPicker) {
    stepsPicker.querySelectorAll('.repeat-opt').forEach(btn => {
      btn.addEventListener('click', () => {
        stepsPicker.querySelectorAll('.repeat-opt').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        paramSteps.value = btn.dataset.steps;
        paramSteps.dispatchEvent(new Event('change'));
      });
    });
  }
  ...
  btnSelectAllEmotions.addEventListener('click', () => {
    document.querySelectorAll('#emotion-checkbox-grid .emotion-pill-chk').forEach(el => {
      ...
      if (input.disabled) return;   // dataset does not carry this emotion
  ...
  function renderTakeResultCell(result) {
    ...
    // Optional pre-SeedVC (VoxCPM2) player: emotional speech still in the donor's
    ...
    const preVcRow = preVcUrl ? `
      <div class="mini-take-player mini-take-prevc" data-url="${preVcUrl}" title="เสียง VoxCPM2 ก่อนผ่าน SeedVC (ยังเป็นเสียง donor)">
        <button type="button" class="btn-mini-play btn-mini-play-prevc" title="เล่นเสียง VoxCPM2 ก่อน VC">
        ...
        <div class="mini-meta"><span class="mini-prevc-label">🎤 VoxCPM2 (ก่อน VC)</span></div>
        <a href="${preVcUrl}" download="${result.pre_vc_filename || 'voxcpm_preVC.wav'}" class="btn-mini-dl" title="ดาวน์โหลด VoxCPM2 ก่อน VC">
    ...
    // VoxCPM2 generation, so the emotion-vs-register trade-off is audible side by side.
    ...
    const modelInputRow = mi ? `
      <button type="button" class="btn-model-input" title="ดู JSON ที่ส่งเข้าโมเดล (VoxCPM2 + SeedVC)">🔧 JSON</button>
