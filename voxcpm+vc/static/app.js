document.addEventListener('DOMContentLoaded', () => {
  ...
  const API_BASE = window.location.protocol === 'file:' ? 'http://127.0.0.1:8013' : '';
  ...
  // Emotion donors
  //
  // The donor set is the actor whose delivery VoxCPM2 clones. One set is one
  // person across all five emotions, so switching sets changes *who* the emotion
  // is performed by, not which emotion is asked for.
  // ---------------------------------------------------------------------- //

  const donorSelect = document.getElementById('donor-select');
  const donorEmotionSelect = document.getElementById('donor-emotion-select');
  const donorAudio = document.getElementById('donor-audio');
  const btnPlayDonor = document.getElementById('btn-play-donor');
  const donorHint = document.getElementById('donor-hint');
  ...
  async function loadDonorSets() {
    ...
    try {
      ...
      const saved = localStorage.getItem('siangtts_donor_set') || data.default || '';
      donorSelect.innerHTML = '<option value="">-- เลือกอัตโนมัติตามเพศ (Auto) --</option>';
      (data.sets || []).forEach(set => {
        const opt = document.createElement('option');
        opt.value = set.id;
        const agree = set.mean_agreement != null
          ? ` · ผู้ฟังตรงกัน ${Math.round(set.mean_agreement * 100)}%`
          : '';
        opt.textContent = `${set.id} (${set.gender})${agree}`;
        // An incomplete set cannot cover every emotion in a script, so say so
        // rather than letting the render fail partway through.
        if (!set.complete) opt.textContent += ' — ไม่ครบ 5 อารมณ์';
        donorSelect.appendChild(opt);
      });
      if (saved) donorSelect.value = saved;

      // Ten annotator tones, five recordings: show what maps onto what so a
      // substituted emotion is never a surprise.
      if (donorHint && data.tone_map) {
        const merged = Object.entries(data.tone_map)
          .filter(([tone, emo]) => tone !== emo)
          .map(([tone, emo]) => `${tone}→${emo}`)
          .join(', ');
        if (merged) {
          donorHint.textContent =
            `อารมณ์มาจากคลิปต้นแบบ ไม่ใช่คำสั่งในวงเล็บ — VoxCPM2 โคลนการออกเสียงจากคลิป ` +
            `แล้ว SeedVC เปลี่ยนน้ำเสียงเป็นเสียงที่เลือกไว้ด้านบน · มีคลิป 5 อารมณ์ ` +
            `ส่วนอารมณ์อื่นถูกจับคู่เป็น: ${merged}`;
        }
      }
    } catch (e) {
      /* the studio still works with the server's automatic pick */
    }
  }

  if (donorSelect) {
    donorSelect.addEventListener('change', () => {
      localStorage.setItem('siangtts_donor_set', donorSelect.value);
    });
  }

  if (btnPlayDonor) {
    btnPlayDonor.addEventListener('click', async () => {
      const set = donorSelect.value;
      const emotion = donorEmotionSelect.value;
      if (!set) {
        alert('เลือกชุดเสียงต้นแบบก่อน จึงจะฟังคลิปได้ (Auto จะเลือกตอนสังเคราะห์)');
        return;
      }
      donorAudio.src = `${API_BASE}/api/donors/${encodeURIComponent(set)}/${encodeURIComponent(emotion)}/audio`;
      try {
        await donorAudio.play();
      } catch (e) {
        alert('ไม่พบคลิปอารมณ์นี้ในชุดที่เลือก');
      }
    });
  }

  function selectedDonorSet() {
    return donorSelect && donorSelect.value ? donorSelect.value : null;
  }
  ...
  async function fetchSynthesisBlob({ text, speakerId, guidance, engine, model, cfgValue, timesteps, loraMode, autoAnnotate = true, postProcess = true, dspParams = null }) {
    if (selectedAudioFile) {
      ...
      const donorSet = selectedDonorSet();
